...
class World:
    ...
    def _assign_source_to_stockpile_haul_task(self, npc: NPC, *, item_key: str = "raw_log") -> bool:
        ...
        if source.get("source_type") == "stockpile":
            ...
            self._record_decision_explanation(explanation_type="actor_skipped", decision="source_skipped", primary_reason="incompatible_stockpile_source_for_source_to_stockpile_flow", actor=npc, contributing_factors={"item_key": item_key})
            return False
        ...
        try:
            ...
            for task in getattr(self, "production_tasks_by_id", {}).values():
                if task.status in {"completed", "failed", "cancelled"}:
                    continue
                tasks.append({"task_id": task.id, "task_type": task.task_type, "status": task.status, "priority": task.priority, "urgency": task.urgency, "blocked_reason": task.blocked_reason, "assigned_actor_ids": list(task.assigned_actor_ids), "reserved_entity_ids": list(task.reserved_entity_ids), "target_entity_id": task.target_entity_id})
            ...
            for actor in getattr(self, "village_npcs", []):
                ...
                actors.append({"actor_id": getattr(actor, "id", None), "current_task": getattr(getattr(actor, "schedule", None), "current_task", None), "task_context": getattr(actor, "task_context", None), "work_identity_label": profile.get("work_identity_label"), "dominant_work_tag": profile.get("dominant_work_tag"), "fatigue_state": profile.get("fatigue_state"), "recent_work_summary": profile.get("recent_work_summary")})
            stockpiles = []
        ...
        for actor in [*getattr(self, "village_npcs", []), *getattr(self, "npcs", [])]:
            ...
            for cf in self.campfires_by_id.values():
                ...
                if dist <= getattr(cf, "warmth_radius", 4):
                    ...
                    break
            if warmed:
                actor.cold_exposure = max(0.0, float(getattr(actor, "cold_exposure", 0.0)) - 0.1)
                ...
                if self.ambient_temperature < self.cold_threshold:
                    ...
                    actor.cold_exposure = min(2.0, float(getattr(actor, "cold_exposure", 0.0)) + delta)
    ...
    def _advance_refill_campfire_fuel_task(self, task: ProductionTask) -> None:
        ...
        if abs(actor.x - cf.x) > 1 or abs(actor.y - cf.y) > 1:
            self._warn_simulation_validation("campfire_refuel_out_of_range", (task.id, actor.id), "Actor not in range to deliver campfire fuel.", actor=actor, metadata={"campfire_id": cf.campfire_id}, cooldown_ticks=120)
            return
