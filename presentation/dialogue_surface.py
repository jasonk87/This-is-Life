...
from entities.social import reinforce_known_fact
...
class DialogueContext:
    ...
    scene_participant_count: int = 0
    shared_activity_type: str | None = None
...
SCENE_TOPIC_BIAS = {
    ...
    "funeral": {"death": 80.0, "grief": 80.0, "respect": 18.0},
    "accusation": {"crime": 65.0, "fear": 45.0},
    "warning": {"fear": 65.0, "crime": 55.0},
    "celebration": {"birth": 58.0, "marriage": 58.0, "migration": 28.0, "respect": 16.0},
    "market_concern": {"employment": 60.0, "migration": 24.0, "curiosity": 18.0},
}
SCENE_TONE_TOPIC_BIAS = {
    "grieving": {"death": 70.0, "grief": 70.0, "respect": 15.0},
    "somber": {"death": 55.0, "grief": 55.0, "respect": 12.0},
    "sad": {"death": 35.0, "grief": 35.0},
    "tense": {"crime": 58.0, "fear": 62.0},
    "fearful": {"crime": 58.0, "fear": 62.0},
    "suspicious": {"crime": 48.0, "fear": 42.0},
    "celebratory": {"birth": 40.0, "marriage": 40.0, "respect": 10.0},
    "concerned": {"employment": 28.0, "migration": 18.0, "curiosity": 12.0},
}
SCENE_ACTIVITY_TOPIC_BIAS = {
    "work": {"employment": 24.0},
    "forging": {"employment": 24.0},
    "mourning": {"death": 30.0, "grief": 30.0},
    "gathering": {"curiosity": 10.0, "marriage": 8.0},
}
SCENE_LINE_POOLS: dict[str, dict[str, tuple[str, ...]]] = {
    "tavern": {
        "crime": ("Over the cups, {confidence_phrase} {subject} was tied to {detail} involving {target}.",),
        "marriage": ("Between drinks, {confidence_phrase} {subject} and {target} married; that will set tongues wagging.",),
        "migration": ("Around here, {confidence_phrase} {subject} has been on the road {detail}.",),
        "curiosity": ("Over the cups, I keep wondering about {subject}.",),
    },
    "worksite": {
        "employment": ("Back to the work of it: {confidence_phrase} {subject} is on {detail} now.",),
        "crime": ("Keep your tools close; {confidence_phrase} {subject} was recorded for {detail} involving {target}.",),
        "curiosity": ("Practical thing is, we still need to know more about {subject}.",),
    },
    "home": {
        "birth": ("At home, {confidence_phrase} there is a new child, {subject}.",),
        "marriage": ("For the household, {confidence_phrase} {subject} and {target} married.",),
        "death": ("Here at home, {confidence_phrase} {subject} is gone now.",),
        "grief": ("This house is quieter; I am grieving for {subject}.",),
        "curiosity": ("Close to home, I keep wondering about {subject}.",),
    },
    "market": {
        "employment": ("In the market talk, {confidence_phrase} {subject} has work as {detail}.",),
        "migration": ("Market news is that {confidence_phrase} {subject} traveled {detail}.",),
        "curiosity": ("The stalls are full of questions about {subject}.",),
    },
    "street": {
        "crime": ("Quick word: {confidence_phrase} {subject} was tied to {detail} involving {target}.",),
        "fear": ("Quick word: I do not feel safe around {subject}.",),
        "migration": ("Quick word: {confidence_phrase} {subject} traveled {detail}.",),
        "curiosity": ("Quick word: people keep asking about {subject}.",),
    },
    "grieving": {
        "death": ("Quietly, {confidence_phrase} {subject} is gone now.",),
        "grief": ("Quietly, the news about {subject} still weighs on me.",),
        "respect": ("Softly, {subject} deserves to be remembered.",),
    },
    "tense": {
        "crime": ("Careful who hears this: {confidence_phrase} {subject} was tied to {detail} involving {target}.",),
        "fear": ("Careful who hears this: I do not feel safe around {subject} now.",),
        "curiosity": ("Careful now, there is more to learn about {subject}.",),
    },
    "celebration": {
        "birth": ("Good news is spreading: {confidence_phrase} there is a new child, {subject}.",),
        "marriage": ("Good news is spreading: {confidence_phrase} {subject} and {target} married.",),
        "migration": ("Everyone is asking about the road: {confidence_phrase} {subject} traveled {detail}.",),
        "respect": ("People are saying {subject} earned some trust.",),
    },
    "market_concern": {
        "employment": ("Market concern is this: {confidence_phrase} {subject} is on {detail} now.",),
        "migration": ("Market concern is this: {confidence_phrase} {subject} traveled {detail}.",),
        "curiosity": ("Market concern keeps circling back to {subject}.",),
    },
}
...
def render_dialogue_line(topic: DialogueTopic, context: DialogueContext) -> DialogueLine:
    ...
    pool = _scene_template_pool(topic_type, context)
    if pool is None:
...
def get_contextual_dialogue_lines(npc, listener, world, limit: int = 3, scene=None) -> list[DialogueLine]:
    """Return deterministic non-LLM lines about what an NPC knows or feels."""
    topics = build_dialogue_topics_from_npc(npc, world)
    context = _build_context(npc, listener, world, topics, scene=scene)
    selected_topics = _select_dialogue_topics(npc, world, topics, max(1, limit), context=context)
...
def build_dialogue_topic_from_fact(fact, world) -> DialogueTopic | None:
    """Build one deterministic dialogue topic from a known structured fact."""
    return _topic_from_fact(fact, world)


def render_history_fact_dialogue_line(npc, listener, world, fact, scene=None) -> DialogueLine | None:
    """Render one non-LLM line for a specific known history fact."""
    topic = build_dialogue_topic_from_fact(fact, world)
    if topic is None:
        return None
    context = _build_context(npc, listener, world, [topic], scene=scene)
    return render_dialogue_line(topic, context)


def remember_dialogue_topic_spoken(npc, topic: DialogueTopic, world) -> None:
    """Record that a dialogue topic was spoken for cooldown purposes."""
    _remember_spoken_topics(npc, [topic], _current_tick(world))


def is_dialogue_topic_on_cooldown(npc, topic: DialogueTopic, world) -> bool:
    """Return whether a topic is still on the speaker's dialogue cooldown."""
    return _topic_is_on_cooldown(topic, _dialogue_memory(npc), _current_tick(world))


def _select_dialogue_topics(npc, world, topics: list[DialogueTopic], limit: int, context: DialogueContext | None = None) -> list[DialogueTopic]:
    if not topics:
        return []
    memory = _dialogue_memory(npc)
    current_tick = _current_tick(world)
    scored_topics = [
        (_topic_score(topic, memory, current_tick, context), topic)
        for topic in topics
...
def _topic_score(topic: DialogueTopic, memory, current_tick: int, context: DialogueContext | None = None) -> float:
    topic_id = _topic_memory_id(topic)
    ...
    score += _scene_topic_bias(topic, context)
...
def _remember_spoken_topics(npc, topics: list[DialogueTopic], current_tick: int) -> None:
    ...
    for topic in topics:
        ...
        knowledge = getattr(npc, "knowledge", None)
        known_facts = getattr(knowledge, "known_history_facts", {}) if knowledge is not None else {}
        if topic.source_id in known_facts:
            reinforce_known_fact(knowledge, topic.source_id, tick=int(current_tick), source_type="direct_conversation")
...
def _build_context(npc, listener, world, topics: list[DialogueTopic], scene=None) -> DialogueContext:
    entity_ids = set()
    for topic in topics:
    ...
    return DialogueContext(
        ...
        scene_type=_scene_type_for(scene),
        scene_tone=_scene_tone_for(scene),
        scene_participant_count=_scene_participant_count_for(scene),
        shared_activity_type=_shared_activity_type_for(scene, npc),
    )


def _scene_type_for(scene) -> str | None:
    scene_type = str(getattr(scene, "scene_type", "") or "").strip().lower()
    return scene_type or None


def _scene_tone_for(scene) -> str | None:
    tone = str(getattr(scene, "tone", "") or "").strip().lower()
    return tone or None


def _scene_participant_count_for(scene) -> int:
    participant_ids = getattr(scene, "participant_ids", None)
...
def _shared_activity_type_for(scene, speaker) -> str | None:
    activity_type = str(getattr(scene, "shared_activity_type", "") or "").strip().lower()
    if activity_type:
        return activity_type
    return _activity_type_for(speaker)



def _scene_topic_bias(topic: DialogueTopic, context: DialogueContext | None) -> float:
    if context is None:
        return 0.0
    bias = 0.0
    scene_type = (context.scene_type or "").lower()
    scene_tone = (context.scene_tone or "").lower()
    activity_type = (context.shared_activity_type or context.current_activity_type or "").lower()
    bias += SCENE_TOPIC_BIAS.get(scene_type, {}).get(topic.topic_type, 0.0)
    bias += SCENE_TONE_TOPIC_BIAS.get(scene_tone, {}).get(topic.topic_type, 0.0)
    for activity_key, activity_bias in SCENE_ACTIVITY_TOPIC_BIAS.items():
        if activity_key in activity_type:
            bias += activity_bias.get(topic.topic_type, 0.0)
    if context.scene_participant_count >= 4 and scene_type in {"tavern", "market", "street"}:
        bias += {"curiosity": 6.0, "migration": 4.0, "marriage": 4.0}.get(topic.topic_type, 0.0)
    return bias


def _scene_template_pool(topic_type: str, context: DialogueContext) -> tuple[str, ...] | None:
    for style in _scene_template_styles(context):
        pool = SCENE_LINE_POOLS.get(style, {}).get(topic_type)
        if pool:
            return pool
    return None


def _scene_template_styles(context: DialogueContext) -> tuple[str, ...]:
    styles: list[str] = []
    scene_tone = (context.scene_tone or "").lower()
    scene_type = (context.scene_type or "").lower()
    if scene_tone in {"grieving", "somber"}:
        styles.append("grieving")
    if scene_tone in {"tense", "fearful", "suspicious"}:
        styles.append("tense")
    if scene_tone == "celebratory":
        styles.append("celebration")
    if scene_type in SCENE_LINE_POOLS:
        styles.append(scene_type)
