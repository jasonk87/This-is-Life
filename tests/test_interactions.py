...
from simulation.systems.scheduling import (
    run_npc_social_gathering_policy,
...
class TestWorldInteractionActions(unittest.TestCase):
    ...
    def test_conversation_topic_selection_for_strangers_biases_shallow_topics(self):
        speaker = engine.NPC(4, 4, name="Speaker")
        listener = engine.NPC(5, 4, name="Listener")
        speaker.social.relationships[listener.id] = 20
        self.world.village_npcs.extend([speaker, listener])
        profile = self.world.evaluate_conversation_foundation(speaker, listener)

        with patch("simulation.systems.conversation_topics.random.choices", return_value=["greeting"]):
            _line, _goal, choice = self.world._resolve_conversation_topic(speaker, listener, profile)

        self.assertIn(choice.topic_type, {"greeting", "small_talk"})

    def test_higher_openness_relationship_can_select_reflection_topic(self):
        speaker = engine.NPC(4, 4, name="Speaker")
        listener = engine.NPC(5, 4, name="Listener")
        speaker.social.relationships[listener.id] = 90
        speaker.knowledge.long_term_memory.append("I still remember when the horse bit me.")
        self.world.village_npcs.extend([speaker, listener])
        profile = self.world.evaluate_conversation_foundation(speaker, listener)

        with patch("simulation.systems.conversation_topics.random.choices", return_value=["reflection"]):
            line, goal, choice = self.world._resolve_conversation_topic(speaker, listener, profile)

        self.assertEqual(choice.topic_type, "reflection")
        self.assertIn("horse bit me", line)
        self.assertEqual(goal, "continue_conversation")

    def test_gossip_topic_selects_known_incident_payload(self):
        speaker = engine.NPC(4, 4, name="Speaker")
        listener = engine.NPC(5, 4, name="Listener")
        victim = engine.NPC(6, 4, name="Victim")
        self.world.village_npcs.extend([speaker, listener, victim])
        incident = engine.create_harmful_incident(
            self.world,
            attacker_id=speaker.id,
            target_id=victim.id,
            location=(4, 4),
            severity=18,
            target_survived=True,
        ...
        engine.record_incident_attribution(speaker, incident, attacker_id=speaker.id, confidence=0.9, basis="actor_self")
        profile = self.world.evaluate_conversation_foundation(speaker, listener)

        with patch("simulation.systems.conversation_topics.random.choices", return_value=["report_incident"]):
            _line, _goal, choice = self.world._resolve_conversation_topic(speaker, listener, profile)

        self.assertEqual(choice.topic_type, "report_incident")
        self.assertEqual(choice.payload.get("incident_id"), incident.id)

    def test_reflection_topic_shares_memory_and_improves_relationship(self):
        speaker = engine.NPC(4, 4, name="Speaker")
        listener = engine.NPC(5, 4, name="Listener")
        speaker.social.relationships[listener.id] = 60
        speaker.knowledge.long_term_memory.append("I once nearly drowned crossing the ford.")
        self.world.village_npcs.extend([speaker, listener])
        profile = self.world.evaluate_conversation_foundation(speaker, listener)
        before = speaker.social.relationships[listener.id]

        with patch("simulation.systems.conversation_topics.random.choices", return_value=["reflection"]):
            line, _goal, _choice = self.world._resolve_conversation_topic(speaker, listener, profile)

        self.assertIn("drowned", line)
        self.assertGreater(speaker.social.relationships[listener.id], before)
        self.assertTrue(any("nearly drowned" in memory for memory in listener.knowledge.long_term_memory))

    def test_player_npc_fallback_dialogue_uses_topic_system(self):
        npc = engine.NPC(4, 4, name="Speaker")
        npc.knowledge.long_term_memory.append("There was a time a horse bit me.")
        npc.social.relationships[self.world.player.id] = 90
        self.world.village_npcs.append(npc)

        with patch("simulation.systems.conversation_topics.random.choices", return_value=["reflection"]):
            line, goal = self.world._fallback_dialogue_continue(npc, "tell me something")

        self.assertIn("horse bit me", line)
        self.assertEqual(goal, "continue_conversation")

    def test_npc_can_gather_around_home_focal_point_with_readable_spacing(self):
        npc = engine.NPC(5, 5, name="Gatherer")
        home = engine.Building(8, 8, 4, 4, "house", global_chunk_x_start=0, global_chunk_y_start=0)
        self.world.buildings_by_id[home.id] = home
        npc.schedule.home_building_id = home.id
        self.world.village_npcs.append(npc)

        with patch("simulation.systems.scheduling.random.random", return_value=0.01), \
             patch("simulation.systems.scheduling._select_social_focal_point", return_value={"kind": "home", "coords": (home.global_center_x, home.global_center_y)}), \
             patch("simulation.systems.scheduling._select_gathering_slot", return_value=(home.global_center_x + 2, home.global_center_y)), \
             patch.object(self.world, "calculate_path", return_value=[(5, 5), (6, 5), (7, 5)]):
            acted = run_npc_social_gathering_policy(self.world, npc)

        self.assertTrue(acted)
        self.assertEqual(npc.schedule.current_task, "gathering_social")
        self.assertIsNotNone(npc.schedule.current_destination_coords)
        distance_to_focal = (
            abs(npc.schedule.current_destination_coords[0] - home.global_center_x)
            + abs(npc.schedule.current_destination_coords[1] - home.global_center_y)
        )
        self.assertGreaterEqual(distance_to_focal, 2)
        self.assertEqual(npc.task_context_data.get("social_context"), "gathering")

    def test_low_pressure_gathering_biases_topic_weights_toward_reflection_and_gossip(self):
        speaker = engine.NPC(4, 4, name="Speaker")
        listener = engine.NPC(5, 4, name="Listener")
        speaker.social.relationships[listener.id] = 80
        speaker.task_context_data = {"social_context": "gathering"}
        speaker.knowledge.long_term_memory.append("I remember the winter flood.")
        self.world.village_npcs.extend([speaker, listener])
        profile = self.world.evaluate_conversation_foundation(speaker, listener)
        captured = {}

        def _capture_choices(topics, weights, k=1):
            for topic, weight in zip(topics, weights):
                captured[topic] = weight
            return ["reflection"]

        with patch("simulation.systems.conversation_topics.random.choices", side_effect=_capture_choices):
            _line, _goal, choice = self.world._resolve_conversation_topic(speaker, listener, profile)

        self.assertEqual(choice.topic_type, "reflection")
        self.assertGreater(captured.get("reflection", 0), captured.get("greeting", 0))
        self.assertGreater(captured.get("small_talk", 0), 0)

    def test_gathering_conversation_report_incident_keeps_real_payload_effects(self):
        speaker = engine.NPC(4, 4, name="Speaker")
        listener = engine.NPC(5, 4, name="Listener")
        victim = engine.NPC(6, 4, name="Victim")
        speaker.task_context_data = {"social_context": "gathering"}
        ...
        engine.record_incident_attribution(speaker, incident, attacker_id=speaker.id, confidence=0.95, basis="actor_self")
        profile = self.world.evaluate_conversation_foundation(speaker, listener)
        ...
        self.assertEqual(choice.topic_type, "report_incident")
        self.assertIn(incident.id, listener.knowledge.known_harmful_incidents)
