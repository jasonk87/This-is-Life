...
ScenarioCallable = Callable[[int, int], ScenarioResult]
...
def run_construction_basic(seed: int, ticks: int) -> ScenarioResult:
    ...
    return ScenarioResult(scenario, seed, ticks, trace)


def run_delivery_basic(seed: int, ticks: int) -> ScenarioResult:
    from engine import ItemReference, NPC
...
def run_hunting_food_chain(seed: int, ticks: int) -> ScenarioResult:
...
def run_settlement_growth(seed: int, ticks: int) -> ScenarioResult:
